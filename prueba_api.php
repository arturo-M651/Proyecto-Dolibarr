...
function callAPI($url, $api_key, $cache_ttl = 300) {
    ...
    if ($httpCode >= 200 && $httpCode < 300 && $response && ($response[0] == '{' || $response[0] == '[')) {
        file_put_contents($cache_file, $response);
    } elseif ($httpCode !== 200 && file_exists($cache_file)) {
        return json_decode(file_get_contents($cache_file), true);
    }
    ...
    <?php else: ?>
        ...
        <div class="row g-4" id="contenedorProductos">
            ...
            if (isset($productos['error'])) {
                echo "<div class='col-12 text-center py-5'><i class='bi bi-exclamation-triangle display-1 text-danger'></i><p class='mt-3 lead'>No pudimos cargar los productos en este momento.</p><small class='text-muted'>Por favor, intenta nuevamente más tarde.</small></div>";
            } elseif (empty($productos)) {
                echo "<div class='col-12 text-center py-5'><i class='bi bi-box-seam display-1 text-muted'></i><p class='mt-3 lead'>No hay productos disponibles en esta categoría.</p></div>";
            } else {
                ...
                <div class="col-6 col-md-4 col-lg-3 item-producto" data-nombre="<?php echo strtolower($label); ?>" data-tipo="<?php echo $es_paquete ? 'paquete' : 'unidad'; ?>" data-aos="zoom-in">
                    <div class="product-card shadow-sm bg-white rounded-4 border-0 h-100 d-flex flex-column">
                        <div class="product-img-wrapper position-relative cursor-pointer" style="height: 220px; overflow: hidden;"
                            ...
                            <img src="<?php echo $img_src; ?>" class="w-100 h-100 object-fit-cover" loading="lazy" onerror="this.src='https://placehold.co/300x300/f8fafc/0e4c81?text=Sin+Foto'">
                                    ...
                                    <div class="tab-pane fade h-100" id="pills-plano">
                                        <div class="card border-0 shadow-sm h-100" id="contenedorCanvas">
                                            ...
                                            <div class="canvas-controls">
                                                <button class="btn-control" id="btnAddMesa" onclick="agregarMesa2D()" title="Añadir Mesa" style="display: none;"><i class="bi bi-plus-circle-dotted"></i></button>
