/**
 * APP.JS - V61 (FIX: INICIALIZACIÓN SEGURA DE MODALES)
 */
...
// INICIALIZACIÓN DIFERIDA (PARA EVITAR ERRORES SI EL HTML NO EXISTE AÚN)
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Inicializar Modales de forma segura
    ...
    if(elDet) modalDetallesBootstrap = new bootstrap.Modal(elDet);
...
// --- FUNCIONES DE NOTAS ESPECIALES (FIX CLIC) ---
function abrirModalNota() {
    if (modalNotaBootstrap) {
        document.getElementById('textoNota').value = ''; 
        modalNotaBootstrap.show();
    } else {
        // Intento de rescate por si no se inicializó al carga
        const elNota = document.getElementById('modalNota');
        if(elNota) {
            modalNotaBootstrap = new bootstrap.Modal(elNota);
            modalNotaBootstrap.show();
        } else {
            alert("Error: No se encuentra el recuadro de notas en el sistema.");
            console.error("Falta el <div id='modalNota'> en prueba_api.php");
        }
    }
}
...
function agregarNotaCarrito() {
    ...
    if (!texto) {
        alert("Por favor escribe qué necesitas.");
        return;
    }
    
    const itemNota = {
        id: 0, 
        nombre: "📝 Solicitud Especial: " + texto,
        precio: 0, 
        cant: 1,
        esNota: true 
    };
...
// ... RESTO DEL CÓDIGO (Intacto, solo asegurando variables globales) ...
...
function renderizar() {
    ...
    carrito.forEach((item, index) => {
        ...
        if(item.esNota) unidad = "(Nota)"; // Identificador visual
